...
class System(Generator):
	def update_role_permissions(self, payload, node_info):
		...
		try:
			...
			for right in role_right_list:
				#Has Race Condition
				permission, created = RolePermission.objects.get_or_create(role=role, role_right=right)
				lgr.info('Permission: %s | Created: %s' % (permission, created))
				if role_action:
					lgr.info('Adding')
	...
	def redirect(self, payload, node_info):
		...
		try:
			...
			elif 'redirect' not in payload.keys() and payload['SERVICE'] in ['LOGIN','CONFIRM ONE TIME PASSWORD','SET PASSWORD','CONFIRM ONE TIME PIN', 'LOGIN SOCIAL', 'REDIRECT TO SETUP BUSINESS']:
