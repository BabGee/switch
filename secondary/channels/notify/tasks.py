...
def _send_outbound_messages(is_bulk=True, limit_batch=100):
	try:
		...
		with transaction.atomic():
			...
			orig_outbound = Outbound.objects.select_for_update(of=('self',)).filter(Q(contact__subscribed=True),Q(contact__product__notification__code__channel__name='WHATSAPP API'),~Q(recipient=None),
				...
				for name,group_df in grouped_df:
					...
					if batch_size>1 and len(group_df.shape)>1 and group_df.shape[0]>1:
						...
						while True:
							...
							lgr.info(payload)
							tasks.append(
				kafka_producer.publish_message(payload['endpoint_url'], None, json.dumps(payload) )
								)
...
def send_outbound_messages():
	_send_outbound_messages(is_bulk=False, limit_batch=60)
...
@transaction.atomic
@single_instance_task(60*10)
def bulk_send_outbound_messages():
	_send_outbound_messages(is_bulk=True, limit_batch=240)
