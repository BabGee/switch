...
async def example_sender_task(app):
    while True:
        ...
        records = await sync_to_async(Outbound.objects.filter(date_created__date=timezone.now().date(), state__name='FAILED').count, thread_sensitive=True)()
        print(f'{records}')
        await hello.send(
...
async def example_sender(app):
    count = time.perf_counter() -s
    elapsed = "{0:.2f}".format(count)
    records = await sync_to_async(Outbound.objects.filter(date_created__date=timezone.now().date()).count, thread_sensitive=True)()
    await hello.send(
        value=Greeting(from_name='Faust Timer', to_name='you', count=float(elapsed), records=records),
    )
