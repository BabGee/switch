		...
		with transaction.atomic():
			def outbound_query():        
				return Outbound.objects.select_for_update(of=('self',)).filter(Q(contact__subscribed=True),Q(contact__product__notification__code__channel__name='SMS'),\
                                                ...
                                                Q(contact__status__name='ACTIVE',contact__product__is_bulk=is_bulk),Q(contact__product__notification__endpoint__name='TEST NOTIFICATION')).order_by('contact__product__priority').select_related('contact').all
