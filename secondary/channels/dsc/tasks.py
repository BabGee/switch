...
class Wrappers:
        ...
        try:
            ...
            if date_filters not in ['', None]:
                ...
                for i in date_filters.split("|"):
                    ...
                    try:
                        date_filter_data[k] = (timezone.now() + timezone.timedelta(days=float(v))).date() if v not in [
                            '', None] else None
                    except Exception, e:
                        lgr.info('Error on date filter: %s' % e)
                if len(date_filter_data):
                    # lgr.info('Date Filter Data: %s' % date_filter_data)
                    for k, v in date_filter_data.items():
                        try:
                            lgr.info('Date Data: %s' % v.isoformat())
                        except:
                            pass
            ...
            if time_filters not in ['', None]:
                for i in time_filters.split("|"):
                    k, v = i.split('%')
                    try:
                        time_filter_data[k] = timezone.now() + timezone.timedelta(hours=float(v)) if v not in ['',
                                                                                                               None] else None
                    except Exception, e:
                        lgr.info('Error on time filter: %s' % e)
            ...
            if institution_filters not in ['', None]:
                for f in institution_filters.split("|"):
                    ...
                    if 'institution_id' in payload.keys() and payload['institution_id'] not in ['', None]:
                        ...
                        if data.pn_data and 'push_notification' in payload.keys() and payload[
            ...
            if data.query.date_values not in [None, '']:
                for i in data.query.date_values.split('|'):
                    ...
                    args.append(k.strip())
                    selected_data[
                        k.strip()] = "to_char(" + data.query.module_name + "_" + data.query.model_name + "." + v + ", 'DD, Month, YYYY')"
            ...
            if data.query.date_time_values not in [None, '']:
                for i in data.query.date_time_values.split('|'):
                    ...
                    args.append(k.strip())
                    selected_data[
                        k.strip()] = "to_char(" + data.query.module_name + "_" + data.query.model_name + "." + v + ", 'DD, Month, YYYY HH:MI:SS TZ')"
                    params['cols'].append({"label": k.strip(), "type": "date", "value": v.strip()})
            ...
            if data.query.month_year_values not in [None, '']:
                for i in data.query.month_year_values.split('|'):
                    ...
                    args.append(k.strip())
                    selected_data[
                        k.strip()] = "to_char(" + data.query.module_name + "_" + data.query.model_name + "." + v + ", 'Month, YYYY')"
                    ...
                    for i in params['cols']:
                        try:
                            if i['value'] == list_data.strip():
                                list_filters_data = report_list.values(list_data.strip()).annotate(
                                    Count(list_data.strip())).order_by(list_data.strip())
                                i['list_filters'] = []
            ...
            if data.query.links not in [None, '']:
                ...
                for i in data.query.links.split('|'):
                    ...
                    href['links'][name] = {"url": "/" + service + "/", "service": service,
                                           "params": data.query.link_params, "icon": icon}
            ...
            if data.pn_data and 'push_notification' in payload.keys() and payload['push_notification'] == True:
                if data.pn_id_field not in ['', None] and data.pn_update_field not in ['', None]:
                    ...
                    report_list_groups = report_list.filter(
                        ~Q(Q(**{data.pn_id_field: None}) | Q(**{data.pn_id_field: ''}))). \
                        filter(date_modified__gte=timezone.now() - timezone.timedelta(minutes=30)). \
                        values(data.pn_id_field).annotate(Count(data.pn_id_field))
    ...
    def balance(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            ...
            float_type_list = manager_list.values('float_type__name', 'float_type__id').annotate(
                count=Count('float_type__id'))
    ...
    def above_60days_defaulters(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            ...
            account_manager = AccountManager.objects.filter(credit=False, credit_paid=False,
                                                            dest_account__account_type__institution=gateway_profile.institution, \
                                                            dest_account__account_type__gateway=gateway_profile.gateway, \
                                                            dest_account__account_type__deposit_taking=False,
                                                            credit_due_date__lt=timezone.now() - timezone.timedelta(
                                                                days=60))
            ...
            for i in account_manager:
                ...
                if i.credit_due_date:
                    item['date_time'] = '%s' % (profile_tz.normalize(i.credit_due_date.astimezone(profile_tz)).strftime(
                        "%d %b %Y %I:%M:%S %p %Z %z"))
    ...
    def credit_account_list(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            ...
            for i in account_manager:
                ...
                item['description'] = 'Interest: %s | Loan %s | Credit Paid: %s | Loan Accounts: %s ' % (
                interest, loan, i['credit_paid'], i['dest_account__id__count'])
    ...
    def credit_account(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            account_manager = AccountManager.objects.filter(credit=False,
                                                            dest_account__account_type__institution=gateway_profile.institution, \
                                                            dest_account__account_type__gateway=gateway_profile.gateway, \
                                                            dest_account__account_type__deposit_taking=False)

            total_amount = Decimal(0)
            credit_details = {'PAID': Decimal(0), 'OUTSTANDING': Decimal(0), 'OVERDUE': Decimal(0),
                              '30 DAYS OVERDUE': Decimal(0), '60 DAYS OVERDUE': Decimal(0)}
            ...
            for i in account_manager:
                ...
                if i.credit_paid:
                    credit_details['PAID'] = credit_details['PAID'] + i.balance_bf
                elif i.credit_due_date and i.credit_due_date < timezone.now() - timezone.timedelta(
                        days=30) and i.credit_due_date > timezone.now() - timezone.timedelta(days=60):
                    credit_details['30 DAYS OVERDUE'] = credit_details['30 DAYS OVERDUE'] + i.balance_bf
    ...
    def investment_summary_chart(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            ...
            investment_list = Investment.objects.all(). \
                extra(select={'month_year': "to_char( amkagroup_co_ke_investment.date_created, 'Month, YYYY')"}).values(
                'investment_type__product_item__product_type__name', 'month_year'). \
                annotate(Count('investment_type__product_item__product_type__name'))

            investment_list_name = investment_list.values('investment_type__product_item__product_type__name').annotate(
                Count('investment_type__product_item__product_type__name'))
            for a in investment_list_name:
                params['cols'].append(
                    {"label": str(a['investment_type__product_item__product_type__name']), "type": "number"})
    ...
    def enrollments_summary_chart(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            ...
            enrollment_list = Enrollment.objects.filter(
                enrollment_type__product_item__institution=gateway_profile.institution, status__name='ACTIVE'). \
                extra(select={'month_year': "to_char( crm_enrollment.date_created, 'Month, YYYY')"}).values(
                'enrollment_type__product_item__product_type__name', 'month_year'). \
            ...
            enrollment_list_name = enrollment_list.values('enrollment_type__product_item__product_type__name').annotate(
                Count('enrollment_type__product_item__product_type__name'))
            for a in enrollment_list_name:
                params['cols'].append(
                    {"label": str(a['enrollment_type__product_item__product_type__name']), "type": "number"})
                init_rows.append(None)
        ...
        try:
            ...
            account_manager = AccountManager.objects. \
                filter(credit=False, dest_account__account_type__institution=gateway_profile.institution,
                       dest_account__account_type__gateway=gateway_profile.gateway,
                       dest_account__account_type__deposit_taking=False). \
                extra(select={'month_year': "to_char( vbs_accountmanager.date_created, 'Month, YYYY')"}).values(
                'balance_bf', 'dest_account__account_type__name', 'month_year')

            # .values('dest_account__id').annotate(Count('dest_account__id')).annotate(Max('id')).values('balance_bf','dest_account__gateway_profile').extra(select={'day': 'date( vbs_accountmanager.date_created )'}).values('dest_account__id','','days')

            account_manager_name = account_manager.values('dest_account__account_type__name').annotate(
                Count('dest_account__account_type__name'))
    ...
    def investment_summary(self, payload, gateway_profile, profile_tz, data):
        ...
        try:
            ...
            investment_type = investment.values('investment_type__id', 'investment_type__name',
                                                'investment_type__value').annotate(Count('investment_type__id'))
            for i in investment_type:
                pie_total = investment.filter(investment_type__id=i['investment_type__id']).annotate(
                    Max('pie')).aggregate(Sum('pie__max'))
        ...
        try:
            ...
            for d in data_list:
                if d.function not in ['', None]:
                    ...
                    # lgr.info('Is a Query: ')
                    try:
                        params, max_id, min_id, t_count, mqtt[d.data_name] = self.report(payload, gateway_profile,
                                                                                         profile_tz, d)
                        cols = params['cols'] if 'cols' in params.keys() else []
                        rowsParams = params['rows'] if 'rows' in params.keys() else []
                        dataParams = params['data'] if 'data' in params.keys() else []
        ...
        try:
            ...
            data_list = DataList.objects.filter(Q(data_name=data_name.strip()), Q(status__name='ACTIVE'), \
                                                ...
                                                Q(Q(access_level=gateway_profile.access_level) | Q(
                                                    access_level=None))).order_by('level')
            ...
            if data_list.exists():
                cols, rows, groups, data, min_id, max_id, t_count, mqtt = self.process_data_list(data_list, payload,
                                                                                                 gateway_profile,
                                                                                                 profile_tz, data)
            else:
                ...
                if 'survey' in data_name:
                    ...
                    if data_name_val and data_name_val == 'Remittance':
                        float_manager = FloatManager.objects.filter(
                            Q(institution=gateway_profile.institution, gateway=gateway_profile.gateway) \
                            | Q(gateway=gateway_profile.gateway, institution=None)). \
                            distinct('float_type__id')
                        collection = {}
                        product_item_list = product_item_list.filter(
                            product_type__in=[f.float_type.float_product_type for f in float_manager])
                    ...
                    elif 'product_item_id' in payload.keys() or (
                            'item' in payload.keys() and 'institution_id' in payload.keys()):
                        if 'product_item_id' in payload.keys():
                            product_item = ProductItem.objects.filter(product_item_id=payload['product_item_id'],
                                                                      status__name='ACTIVE')
                        else:
                        ...
                        item = {"index": i.id, "name": name, "type": itype,
                                ...
                                "href": {
                                    ...
                                    "Add New Document Requirement": {
                                        "url": "/bid_document_requirement/?bid_id=" + str(i.id),
                                        "params": {"bid_id": str(i.id)},
                    ...
                    from thirdparty.bidfather.models import Bid, BidApplication, BidRequirement, BidDocumentApplication, \
                        BidDocument, BidInvoice, BidRequirementApplication
                    bid_requirement_list = BidRequirement.objects.filter(bid__id=payload['bid_id'])
                    ...
                    # query filters
                    if "q" in payload.keys() and payload['q'] not in ["", None]:
                        bid_applications_list = bid_applications_list.filter(
                            description__icontains=payload['q'].strip())
...
def process_push_notification():
    try:
        ...
        gateway_profile_list = GatewayProfile.objects.filter(access_level__name='SYSTEM', status__name='ACTIVATED',
                                                             user__username='MQTTUser')
        for gateway_profile in gateway_profile_list:
            profile_tz = pytz.timezone(gateway_profile.user.profile.timezone)

            data_list = DataList.objects.filter(Q(status__name='ACTIVE'), Q(pn_data=True), \
                                                Q(Q(gateway=gateway_profile.gateway) | Q(gateway=None))).order_by(
                'level')
            ...
            if data_list.exists():
                ...
                cols, rows, groups, data, min_id, max_id, t_count, mqtt = Wrappers().process_data_list(data_list,
                                                                                                       payload,
                                                                                                       gateway_profile,
                                                                                                       profile_tz, data)
                    ...
                    if len(value):
                        ...
                        for k, v in value.items():
                            try:
                                ...
                                lgr.info('MQTT update Failure ' + e.message)
                        # disconnect after loop
                        msc.disconnect()
    except Exception, e:
        lgr.info('Error on process push notification: %s' % e)