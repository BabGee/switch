...
class System(Wrappers):
	...
	def loan_notification_details(self, payload, node_info):
		try:
			...
			session_manager_list = AccountManager.objects.filter(id__gte=payload['account_manager_id'],credit=False,\
						dest_account=session_manager.dest_account, dest_acount__account_type=session_manager.dest_account.account_type,\
	...
	def debit_account(self, payload, node_info):
		try:
			...
			account_type = None
			if 'account_type_id' in payload.keys() and 'currency' not in payload.keys():
				account_type = AccountType.objects.get(id=payload['account_type_id'])
				payload['currency'] = account_type.product_item.currency.code

			elif 'currency' not in payload.keys():
				payload['currency'] = 'KES'

			if 'account_type' in payload.keys():
				account_type = AccountType.objects.get(Q(name=payload['account_type']),Q(product_item__currency__code=payload['currency']),\
						Q(gateway=gateway_profile.gateway)|Q(gateway=None))

			elif 'account_type_id' in payload.keys() and account_type is None:
				account_type = AccountType.objects.get(id=payload['account_type_id'])

			else:
				account_type = AccountType.objects.filter(product_item__currency__code=payload['currency'], gateway=gateway_profile.gateway)[:1][0]


			#Ensure Branch does not conflict to give more than one result
			gl_account_type = AccountType.objects.get(product_item__institution_till=session_account.account_branch,\
			...
			gl_acccount = Account.objects.filter(account_branch=session_account.account_branch,account_status__name='ACTIVE',account_type=gl_account_type)



			session_account_manager = AccountManager.objects.filter(dest_account = session_account,dest_account__account_type=account_type).order_by('-date_created')
			gl_account_manager = AccountManager.objects.filter(dest_account = gl_acccount[0],dest_account__account_type=gl_account_type).order_by('-date_created')


			#Get Charge amount and transfer charge amount to charge account (just like MIPAY LEDGER)
			charge = Decimal(0)
			charge_list = AccountCharge.objects.filter(account_type=account_type, min_amount__lte=Decimal(payload['amount']), service__name=payload['SERVICE'],\
			...
			if ( Decimal(session_balance_bf) <= Decimal(account_type.max_balance) and Decimal(session_balance_bf) >= Decimal(account_type.min_balance) ) or credit_overdue:
	...
	def credit_account(self, payload, node_info):
		try:
			...
			if 'account_type' in payload.keys():
				account_type = AccountType.objects.get(name=payload['account_type'],product_item__currency__code=payload['currency'],\
						gateway=gateway_profile.gateway)
			...
			#Ensure Branch does not conflict to give more than one result
			gl_account_type = AccountType.objects.get(Q(product_item__institution_till=session_account.account_branch),\
						Q(product_item__currency__code=payload['currency']),Q(product_item__product_type__name='Ledger Account'),\
						Q(gateway=gateway_profile.gateway)|Q(gateway=None))


			gl_acccount = Account.objects.filter(account_branch=session_account.account_branch,account_status__name='ACTIVE',account_type=gl_account_type)

			session_account_manager = AccountManager.objects.filter(dest_account = session_account,account_type=account_type).order_by('-date_created')
			gl_account_manager = AccountManager.objects.filter(dest_account = gl_acccount[0],account_type=gl_account_type).order_by('-date_created')
			...
			if Decimal(session_balance_bf) <= Decimal(account_type.max_balance) and Decimal(session_balance_bf) >= Decimal(account_type.min_balance):
	...
	def get_account(self, payload, node_info):
		try:
			...
			if 'msisdn' in payload.keys():
				session_account = Account.objects.filter(Q(account_status__name='ACTIVE'),Q(gateway_profile__msisdn__phone_number=payload['msisdn']),\
						Q(gateway_profile__gateway=gateway_profile.gateway)|Q(gateway_profile__gateway__name='MIPAY'))

			elif 'email' in payload.keys() and payload['email'] not in [None,""] and self.validateEmail(payload['email']):
				session_account = Account.objects.filter(Q(account_status__name='ACTIVE'),Q(gateway_profile__user__email=payload['email']),\
						Q(gateway_profile__gateway=gateway_profile.gateway)|Q(gateway_profile__gateway__name='MIPAY'))
			...
			if session_account.exists():
				...
				f_session_account = session_account.filter(is_default=True)

				if len(f_session_account)>0:
					payload['session_account_id'] = f_session_account[0].id
				else:
				...
				if 'account_type' in payload.keys():
					account_type = AccountType.objects.get(Q(name=payload['account_type']),Q(product_item__currency=currency),\
					...
					account_type = AccountType.objects.get(Q(product_item__currency=currency),\
							Q(gateway=session_gateway_profile.gateway),\
							~Q(product_item__product_type__name='Ledger Account'))

				session_account = Account(gateway_profile=session_gateway_profile,\
						account_branch=till,account_status=status, account_type=account_type)
...
class Payments(System):
	...
	def loan_status(self, payload, node_info):
		try:
			...
			account_manager_list = AccountManager.objects.filter(credit=False,dest_account=account,account_type__id=payload['account_type_id'],credit_paid=False)
			...
			if account_manager_list.exists():
				...
				currency = account.account_type.currency.code
				for a in account_manager_list:
	...
	def loan_limit(self, payload, node_info):
		try:
			account = Account.objects.get(id=payload['session_account_id'])
			account_manager = AccountManager.objects.filter(credit=False,dest_account=account,desct_account__account_type__id=payload['account_type_id'],credit_paid=False).order_by('-date_created')[:1]
