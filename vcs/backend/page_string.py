		...
		for v in variables:
			...
			if variable_key is not None:
				...
				if variable_key == 'RESPONSE_SUMMARY':
					from administration.models import ResponseStatus
				...
				if variable_key == 'SELECTION':
					...
					from regix.models import Product
					...
					from bridge.models import EnrolledService
					...
					from regix.models import EnrolledService
					...
					from notify.models import Contact
					...
					from vbs.models import AccountType,AccountManager
					...
					for i in payment_method:
						...
						if i.name == 'MIPAY' and variable_val <> 'Send':
					...
					from vbs.models import AccountManager
					from pos.models import BillManager
					...
					from vbs.models import AccountManager
					from muziqbit.models import Music
					...
					from vbs.models import CreditType
					...
					from vbs.models import Account, CreditType

					params = self.get_nav(navigator)

					account_list = Account.objects.filter(gateway_profile__msisdn__phone_number=payload['msisdn'],account_status__name='ACTIVE',\
					...
					from crm.models import ProductItem
					from vbs.models import AccountManager
					...
					from vbs.models import Account
					params = self.get_nav(navigator)

					account_list = Account.objects.filter(Q(gateway_profile__msisdn__phone_number=payload['msisdn']),Q(account_status__name='ACTIVE'),\
									Q(account_type__id=params['account_type_id']),\
									Q(account_type__gateway =code[0].gateway)|Q(account_type__gateway=None)).order_by('account_type__name')
					...
					if account_list.exists():
		                                amount = '{0:,.2f}'.format(account_list[0].credit_limit) 
					...
					from vbs.models import AccountType, AccountCharge
					params = self.get_nav(navigator)
					...
					if 'payment_method' in params.keys():
						lgr.info('Charge List: %s|%s' % (charge_list, params))
						charge_list = charge_list.filter(payment_method__name=str(params['payment_method']))
						lgr.info('Charge List: %s|%s' % (charge_list, params))
					...
					from pos.models import PurchaseOrder
					...
					from crm.models import Enrollment, ProductItem
					...
					from muziqbit.models import Download
