...
from vcs.backend.page_string import PageString
	...
	def initialize(self, *args):
		...
		if 'mno' in args:
			...
			if len(self.navigator) > 0 and self.payload['input']<>'00':#Not a Main Menu Request
				...
				#Initiate Session
				self.session = SessionHop(session_id=self.payload['sessionid'], channel=self.channel, reference=self.payload['msisdn'])
				if len(self.gateway_profile)>0:
