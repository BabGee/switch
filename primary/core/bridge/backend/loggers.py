...
class Loggers:
	...
	def log_transaction(self, service, gateway_profile, payload):
		try:
			...
			channel = Channel.objects.using('default').get(id=int(payload['chid']))
			currency_code = payload['currency'] if 'currency' in payload.keys() and payload['currency']!='' else None
			currency = Currency.objects.using('default').get(code=currency_code) if currency_code is not None  else None
			...
			transaction_status = TransactionStatus.objects.using('default').get(name='CREATED')
			response_status = ResponseStatus.objects.using('default').get(response='DEFAULT')
			...
			if "msisdn" in payload.keys():
				...
				if len(msisdn) >= 9 and msisdn[:1] == '+':
					...
					country_list = Country.objects.using('default').filter(mpoly__intersects=trans_point)
					...
					if country_list.exists() and country_list[0].ccode:
						...
						country_list = Country.objects.using('default').filter(mpoly__intersects=ip_point)
						if country_list.exists() and country_list[0].ccode:
							msisdn = '+%s%s' % (country_list[0].ccode,msisdn[1:])
						else:
							msisdn = None
			...
			if msisdn is not None:
				try:msisdn = MSISDN.objects.using('default').get(phone_number=msisdn)
				except MSISDN.DoesNotExist: msisdn = MSISDN(phone_number=msisdn);msisdn.save(using='default');
				trans.msisdn = msisdn

			if 'institution_id' in payload.keys():
				trans.institution = Institution.objects.using('default').get(id=payload['institution_id'])
	...
	def update_transaction(self, trans, payload, response):
		try:
			...
			trans.response_status = ResponseStatus.objects.using('default').get(response=str(response['response_status']))	
			trans.transaction_status = TransactionStatus.objects.using('default').get(name='PROCESSED')
			trans.overall_status = ResponseStatus.objects.using('default').get(response=str(response['overall_status']))	
