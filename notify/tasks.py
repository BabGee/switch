...
def send_outbound_sms_messages():
	...
	#Outbound.objects.filter(id__in=list(outbound.values_list('id',flat=True))).update(state=OutBoundState.objects.get(name='PROCESSING'), sends=F('sends')+1)
	#outbound = outbound.values('id','message','contact__gateway_profile__msisdn__phone_number')
