...
class Wrappers:
	...
	@app.task(filter=task_method, ignore_result=True)
	def send_outbound(self, i, payload, node):
		from celery.utils.log import get_task_logger
		lgr = get_task_logger(__name__)
		try:
...
class System(Wrappers):
	def add_notification_contact(self, payload, node_info):
		try:
			...
			if 'msisdn' in payload.keys():
				...
				notification_product = notification_product.filter(notification__code__mno=prefix[0].mno)
	...
	def recipient_to_msisdn(self, payload, node_info):
		try:
			...
			if 'recipient' in payload.keys() and payload['recipient'] not in ["",None]:
				...
				msisdn = msisdn.strip()
				msisdn = '+%s' % msisdn if '+' not in msisdn else msisdn #clean msisdn for lookup
				if len(msisdn) >= 10 and msisdn[:1] == '+':
					msisdn = str(msisdn)
				else:
					msisdn = None
		...
		try:
			...
			if len(notification_product)>0:
				...
				if 'message' not in payload.keys():
					...
					template = NotificationTemplate.objects.filter(product=notification_product[0],service__name=payload['SERVICE'])

					if template.exists():
						payload['notification_template_id'] = template[0].id
						message = template[0].template_message
				...
				try: del payload['product_item_id'] #Avoid deduction of float
				except: pass
	...
	def create_sms_notification(self, payload, node_info):
		try:
			...
			lgr.info('Contact: %s' % contact)
			#get template using service & get/create contact
			template = NotificationTemplate.objects.filter(product=notification_product[0],service__name=payload['SERVICE'])
			if template.exists():
				payload['notification_template_id'] = template[0].id
				details = json.dumps({})
	...
	def log_outbound_message(self, payload, node_info):
		try:
...
def send_outbound_sms_messages():
	...
	outbound = Outbound.objects.select_for_update().filter(Q(contact__subscribed=True),Q(contact__product__notification__code__channel__name='SMS'),\
				...
				prefetch_related('contact').select_related('contact__gateway_profile','contact__product','contact__product__notification','contact__product__notification__code')[:250]
	...
	for i in outbound:
		try:
			...
			msisdn = i.contact.gateway_profile.msisdn
			if msisdn not in ['',None] and len(msisdn.phone_number)>=9 and len(msisdn.phone_number)<=15:
				payload = {}
				payload['kmp_correlator'] = i.id
				payload['kmp_message'] = message
				payload['kmp_recipients'] = [str(msisdn.phone_number)]
				payload['kmp_spid'] = endpoint.account_id
				payload['kmp_password'] = endpoint.password
				...
				payload['node_account_id'] = endpoint.account_id
				payload['node_username'] = endpoint.username
				payload['node_password'] = endpoint.password
				payload['contact_info'] = json.loads(i.contact.subscription_details)

				if i.contact.linkid is not None:
					payload['linkid'] = i.contact.linkid

				#Send SMS
				node = endpoint.url
				#No response is required on celery use (ignore results)
				Wrappers().send_outbound.apply_async((i, payload, node))
			else:
				lgr.info('No Valid MSISDN')
		except Exception, e:
	...
	for c in your_list:
		try:
			...
			if c[1] not in ['',None]:
				payload['FULL_NAMES'] = c[1].strip()
			if c[2] not in ['',None]:
				payload['dob'] = c[2].strip()
			if c[3] not in ['',None]:
				payload['enrollment_date'] = c[3].strip()
			gateway_profile = GatewayProfile.objects.get(id=223057)
			service = Service.objects.get(name=service)
			try:Wrappers().service_call(service, gateway_profile, payload) #No async as float deduction needs sync
			except Exception, e: lgr.info('Error on Service Call: %s' % e)
